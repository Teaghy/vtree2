...
> 此项目基于wsfe/vue-tree, 根据自身的业务需求, 仅对部分功能做了调整或新增. 
> 此项目是vue2版本的虚拟树，且仅支持vue2版本
...
| onDuplicateKey                   | 出现相同的keyField的处理方式, 默认抛出异常                                        | `'throw' \| 'warn' `                                                                                             | throw                                                                                  |
| separator                        | 多选模式下 value 分隔符                                                   | `string`                                                                                                         | ', '                                                                                   |
| multiple                         | 是否允许节点多选                                                          | `boolean`                                                                                                        | false                                                                                  |
| checkable                        | 是否可多选                                                             | `boolean`                                                                                                        | false                                                                                  |
| selectable                       | 是否可单选                                                             | `boolean`                                                                                                        | false                                                                                  |
| selected-keys(.sync)             | 如果设定则 selected 状态受控                                               | `Array<string \| number>`                                                                                        | []                                                                                     |
| checked-keys(v-model)            | 如果设定则 checked 状态受控                                                | `Array<string \| number>`                                                                                        | []                                                                                     |
...
| expandedKeys(.sync) `2.2.0`      | 展开的节点 key ，组件内部将会响应此 Prop 的变化                                     | `Array<string \| number>`                                                                                        | []                                                                                     |
...
| check            | 点击复选框触发        | function(node, e:{checked: bool, checkedNodes, checkedKeys})    |
| uncheck          | 取消勾选时触发（多选）    | 被取消勾选的节点信息                                                      |
| checked-change   | 勾选/取消勾选时触发（多选） | 所有被勾选节点（数组）                                                     |
| select           | 选择时触发          | function(node, e:{selected: bool, selectedNodes, selectedKeys}) |
...
| before-expand    | 节点展开之前触发       | 节点信息                                                            |
| after-expand     | 节点展开之后触发       | `void` （注：如果有展开动画， 会在展开动画完成后触发）                                 |
...
| setSelectedKeys        | 设置多选选中/取消选中                | `keys: Array<string \| number>` : 节点 key<br/> `value: boolean` : 是否选中                                                                                                  |                            |
...
| switcherIcon | 自定义节点的展开/折叠图标，slotProps 为 `{ node: TreeNode }` |
