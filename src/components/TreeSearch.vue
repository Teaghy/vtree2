...
import type { ApiType, TreeNodeKeyType } from '../const';
...
const methods: { [key in keyof CTreeApiMethodsType]: CTreeApiMethodsType[key] } = (CTree as any)
  .options.methods;
for (let key in methods) {
  ...
  if (API_METHODS.indexOf(keyCache) > -1) {
    ...
    _methods[keyCache] = function <T extends typeof ctreeMethods[typeof keyCache]>(
      ...args: Parameters<T>
    ): ReturnType<T> {
...
export default (
      ...
      return [
        ...
        `${treeNodePrefixCls}__checkbox`,
        {
          ...
          [`${treeNodePrefixCls}__checkbox_disabled`]:
            this.searchDisabled || this.checkAllStatus.disabled,
        },
      ];
    },
  ...
  methods: {
    ...
    handleCheckAll(): void {
      ...
      const currentVisibleKeys = this.$refs.tree
        .getCurrentVisibleNodes()
        .map((node) => node[this.$refs.tree.keyField]);
