...
import type { TreeNodeKeyType, IgnoreType, VerticalPositionType } from '../const';
import { ignoreEnum, verticalPositionEnum, dragHoverPartEnum } from '../const';
...
const sameValue = (newVal: VModelType, valueCache: VModelType): boolean => {
  if (Array.isArray(newVal) && Array.isArray(valueCache)) {
    if (
      ...
      newVal.every((val) => (valueCache as TreeNodeKeyType[]).some((cache) => cache === val))
    )
      return true;
...
export default (
  ...
  props: {
    ...
    value: [String, Number, Array as () => TreeNodeKeyType[]],
  ...
  methods: {
    ...
    showCheckedNodes(showUnloadCheckedNodes?: boolean): void {
      ...
      showUnloadCheckedNodes =
        showUnloadCheckedNodes == null ? this.showUnloadCheckedNodes : showUnloadCheckedNodes;
    ...
    handleNodeCheck(node: TreeNode): void {
      ...
      this.nonReactive.store.setChecked(
        node[this.keyField],
        ...
        try {
          ...
          if (shouldDrop) {
            ...
            if (hoverPart === dragHoverPartEnum.before)
              this.nonReactive.store.insertBefore(targetKey, referenceKey);
            // 如果是拖拽到父节点，并且父节点是展开的，则不管 hoverPart 是不是 after 都拖入为子节点
            else if (hoverPart === dragHoverPartEnum.body || (!data.isLeaf && data.expand))
              this.nonReactive.store.prepend(targetKey, referenceKey);
            else if (hoverPart === dragHoverPartEnum.after)
              this.nonReactive.store.insertAfter(targetKey, referenceKey);
      ...
      for (let event in this.$listeners) {
        if (storeEvents.indexOf(event as keyof IEventNames) > -1) {
          ...
          this.nonReactive.store.on(
            e,
            this.$listeners[event] as ListenerType<typeof e> | Array<ListenerType<typeof e>>
          );
        }
      }
    },
    ...
    updateRenderAmount(): void {
      ...
      this.renderAmount = Math.max(
        ...
        Math.ceil(clientHeight / this.nodeMinHeight) + this.bufferNodeAmount
      );
    },
    /**
    ...
    updateRenderNodes(isScroll: boolean = false): void {
      if (this.blockLength > this.renderAmount) {
        ...
        this.renderStart =
          Math.floor(scrollNodeAmount / this.bufferNodeAmount) * this.bufferNodeAmount;
      } else {
        this.renderStart = 0;
      }
      if (
        ...
        return;
      this.renderNodes = this.nonReactive.blockNodes
        .slice(this.renderStart, this.renderStart + this.renderAmount)
        .map((blockNode) => {
      ...
      this.bottomSpaceHeight =
        this.blockAreaHeight - (this.topSpaceHeight + this.renderNodes.length * this.nodeMinHeight);
    },
