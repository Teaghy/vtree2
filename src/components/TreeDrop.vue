...
import type { PlacementType, ApiType, TreeSearchApiType, TreeNodeKeyType } from '../const';
import { API_METHODS, TREE_SEARCH_API_METHODS, placementEnum } from '../const';
...
const methods: { [key in keyof CTreeSearchApiMethodsType]: CTreeSearchApiMethodsType[key] } = (
  CTreeSearch as any
).options.methods;
for (let key in methods) {
  ...
  if (COMBINED_API_METHODS.indexOf(keyCache) > -1) {
    ...
    _methods[keyCache] = function <T extends typeof ctreeSearchMethods[typeof keyCache]>(
      ...args: Parameters<T>
    ): ReturnType<T> {
      return (methods as any)[keyCache].apply(
        (this as any).$refs.treeSearch as CTreeSearchInstanceType,
        args
      );
    };
  }
...
export default (
  ...
  computed: {
    ...
    dropdownCls(): string[] {
      const extraClassName = Array.isArray(this.dropdownClassName)
    ...
    displayValue(): string | TreeNodeKeyType {
      if (this.checkable) {
        if (this.checkedCount === 0 && typeof this.dropPlaceholder === 'string')
          return this.dropPlaceholder;
        return `已选 ${this.checkedCount} 个`;
      } else if (this.selectable) {
        if (this.selectedTitle === '' && typeof this.dropPlaceholder === 'string')
          return this.dropPlaceholder;
  ...
  methods: {
    ...
    locateDropdown(): void {
      ...
      // Set dropdown width
      const minWidth = `${
        typeof this.dropdownMinWidth === 'number' ? this.dropdownMinWidth : referenceWidth
      }px`;
    ...
    handleDocumentClick(e: MouseEvent): void {
      if (!this.$refs) return;
      if (
        !this.$refs.reference.contains(e.target as Node) &&
        !this.$refs.dropdown.contains(e.target as Node)
      ) {
